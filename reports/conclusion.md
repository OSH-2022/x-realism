...
rCore 已经完成的工作有：进程概念管理、段页式内存管理、基于文件概念的 IPC、文件系统。本项目将以 rCore 实现到文件系统的代码为基本框架，逐步删去其对文件系统的依赖，并在此基础上实现内核的 Basic IPC。
...
**地址空间**：内核在引导到第一个用户进程后，通过将相应的上限存放在根任务的 Cspace 中，将所有空闲内存交给用户空间来控制系统资源。然后根任务可以实施其资源管理策略，例如通过将系统划分为安全域并将每个域交给一个不相交的无类型内存子集。用户空间可直接访问的唯一对象是“框架对象”：这些对象可以被映射到页表，之后用户空间可以写入由这些框架对象表示的物理内存。简而言之，seL4 将内核资源的管理导出到用户级别，并使它们受到与用户资源相同的基于能力的访问控制。
...
**生产力**：Rust 有内容详尽的**文档**以及开放、友好、高效的**开源社区**。并且有一流的开发**工具链**。
...
- rCore 在实现 IPC 之前就已经有了文件的抽象，但是我们的微内核设计中的 IPC 原语应当不依赖于文件的抽象。下面是一些可选的方案：
  ...
  - 提供一个缺省文件系统。当上位文件系统不存在时，这个文件系统生效。

### 3.2 设计方案

首先给出我们微内核操作系统的整个设计结构：

![image-20220629104124805](C:/Users/10258/Desktop/pic/image-20220629104124805.png)
...
- 针对 IPC 开销较大的问题，我们：
  ...
  - 其次将决定权交给用户，让用户根据自己的操作系统要求（从安全、性能和即时性方面考虑）自选安装模式。如针对计算密集场景，用户可选择禁用时间片轮转的滴答中断。在不联网的纯计算场景下，可不考虑一些安全性问题，让各 Server 的地址空间重合。
...
进阶目标是封装成函数库的形态，为内核支持函数库的形态，基于 `RustSBI` 完成输出及关机等。

#### 3.3.2 进程调度和 IPC

进程调度和 IPC 是我们第一步要添加的内容。高性能和多任务并发的支持是本项目内核计划实现的两个特性，因此进程调度和 IPC 显得尤为重要。
...
我们计划利用上述特性实现我们自己的进程调度和 IPC 模块。包括常规的进程管理，创建进程、销毁进程、进程等待；进程调度相关的算法实现；进程间通信，上下文切换的高效实现。
...
在 rCore 项目基础上（宏内核、RISC-V架构、已实现大部分核心的操作系统功能）整合我们自己的 IPC 模式。同时这一步引入 rCore 的高效内存管理和进程控制模型，减少工作负担。

#### 3.3.4 模块化文件系统和并发测试

因为是微内核设计，文件系统之类的服务都被隔离成模块。为了测试我们的微内核设计，我们将文件系统拆解成模块，并重新测试文件系统的功能。

验证我们系统的并发性，目前一个方案是参考 Redox，运行 rusthello（一个高度并发的 AI）。
